...
Future<List<Item>> fetchItems() async {
  final response = await http.get(Uri.parse('http://192.168.86.31/3GXInventory/php/erpGetItem.php'));
...
Future<int> fetchCount(String itemNo, String branchNo) async {
  var url = Uri.parse('http://192.168.86.31/3GXInventory/php/erpGetItem.php');
...
Future<int> fetchInventoryBranchCount(String itemNo, String branchNo) async {
  final response = await http.get(
    Uri.parse('http://192.168.86.31/3GXInventory/php/fetch_inventory_branch.php')
  );
...
class Item {
  ...
  factory Item.fromJson(Map<String, dynamic> json) {
    ...
    return Item(
      ...
      imageFilename: json['Item_Pix_Filename'] ?? '',
    );
  }
}